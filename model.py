...
class Resnet50Encoder_at(nn.Module):
    def __init__(self, pretrained=True):
        super(Resnet50Encoder_at, self).__init__()
        ...
        self.o1 = nn.Sequential(*list(resnet.children())[1:-5])
        self.o2 = nn.Sequential(*list(resnet.children())[1:-4])
        self.o3 = nn.Sequential(*list(resnet.children())[1:-3])
        self.o4 = nn.Sequential(*list(resnet.children())[1:-2])
        self.feature = nn.Sequential(*list(resnet.children())[1:-1]) # before avgpool
...
class Resnet101Encoder_at(nn.Module):
    def __init__(self, pretrained=True):
        super(Resnet101Encoder_at, self).__init__()
...
class Resnet18_bosphorus(nn.Module):
    def __init__(self, pretrained=True):
        super(Resnet18_bosphorus, self).__init__()
        ...
        self.features = nn.Sequential(*list(resnet.children())[1:-1])  # after avgpool 512x1

        # fc_in_dim = list(resnet.children())[-1].in_features  # original fc layer's in dimention 512
        #
        self.fc1 = nn.Linear(512, 128)
        self.drop1 = nn.Dropout(0.5)
        self.fc2 = nn.Linear(128, 6)  # new fc layer 512x7
        # self.alpha = nn.Sequential(nn.Linear(fc_in_dim, 1), nn.Sigmoid())
...
class Resnet18(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # fc_in_dim = list(resnet.children())[-1].in_features  # original fc layer's in dimention 512
        #
        self.fc = nn.Linear(512, 16)  # new fc layer 512x7
        # self.alpha = nn.Sequential(nn.Linear(fc_in_dim, 1), nn.Sigmoid())
...
class Resnet34(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # fc_in_dim = list(resnet.children())[-1].in_features  # original fc layer's in dimention 512
        #
        self.fc = nn.Linear(512, 6)  # new fc layer 512x7
        # self.alpha = nn.Sequential(nn.Linear(fc_in_dim, 1), nn.Sigmoid())
...
class Resnet50(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.conv1 = nn.Conv2d(8, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        self.features = nn.Sequential(*list(resnet.children())[1:-1])  # after avgpool 2048x1



        self.fc = nn.Linear(2048, 20)  # new fc layer
...
class Resnet101(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # fc_in_dim = list(resnet.children())[-1].in_features  # original fc layer's in dimention 512
        #
        self.fc = nn.Linear(2048, 16)  # new fc layer 512x7
        # self.alpha = nn.Sequential(nn.Linear(fc_in_dim, 1), nn.Sigmoid())
